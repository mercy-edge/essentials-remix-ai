var PokedexPokemonPanel = PokedexResultPanel.extend({
	initialize: function(id) {
		...
		if (window.BattleEncounters && BattleEncounters[id] && BattleEncounters[id].length) {
			buf += '<h3>Wild encounters</h3>';
			buf += '<table class="stats"><tr><th>Location</th><th>Method</th><th>Rarity</th><th>Levels</th></tr>';
			for (var ei = 0; ei < BattleEncounters[id].length; ei++) {
				var enc = BattleEncounters[id][ei];
				buf += '<tr><td>' + Dex.escapeHTML(enc.map) + '</td>';
				buf += '<td>' + Dex.escapeHTML(enc.method) + '</td>';
				buf += '<td>' + Dex.escapeHTML(enc.rarity) + '</td>';
				buf += '<td>' + Dex.escapeHTML(enc.levels) + '</td></tr>';
			}
			buf += '</table>';
		}
