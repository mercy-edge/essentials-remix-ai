...
#-------------------------------------------------------------------------------
# Apply Gen 9 Auto-Updates to base PBS (abilities/items/moves/species).
# Safe for custom PBS content — only overwrites listed Auto-Updates lines.
#-------------------------------------------------------------------------------
MenuHandlers.add(:debug_menu, :gen9_update_pbs, {
  "name"        => _INTL("Apply Gen 9 PBS updates"),
  "parent"      => :editors_menu,
  "description" => _INTL("Run Compiler.update_gen9 (evolutions, move flags, etc.)."),
  "effect"      => proc {
    if pbConfirmMessage(_INTL("Update base PBS with Gen 9 Auto-Updates?\n(Custom lines outside those files are kept.)"))
      Compiler.update_gen9
      pbMessage(_INTL("Gen 9 PBS updates applied. Recompile if prompted."))
    end
  }
})
