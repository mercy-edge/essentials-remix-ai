class Battle
  ...
  def pbEOREndWeather(priority)
    ...
    when :Snow        then pbDisplay(_INTL("The snow is falling deep."))
  ...
  def pbEORWeatherDamage(battler)
    ...
    when :Snow
      # Snow replaces hail here; no chip damage (Gen 9-style).
      return
    when :ShadowSky
  ...
  def pbEOREffectDamage(priority)
    ...
    # Salt Cure
    priority.each do |battler|
      next if battler.effects[PBEffects::SaltCure] < 0 || !battler.takesIndirectDamage?
      amt = battler.totalhp / 16
      amt = battler.totalhp / 8 if battler.pbHasType?(:STEEL) || battler.pbHasType?(:WATER)
      battler.pbTakeEffectDamage(amt) do |_hp_lost|
        pbDisplay(_INTL("{1} is hurt by Salt Cure!", battler.pbThis))
      end
    end
    # Rend bleed
    priority.each do |battler|
      next if battler.effects[PBEffects::RendBleed] <= 0 || !battler.takesIndirectDamage?
      battler.pbTakeEffectDamage([battler.totalhp / 16, 1].max) do |_hp_lost|
        pbDisplay(_INTL("{1} is hurt by bleeding!", battler.pbThis))
      end
      battler.effects[PBEffects::RendBleed] -= 1
    end
    # Syrup Bomb (Speed drops at end of each turn while syrup remains)
    priority.each do |battler|
      next if battler.fainted? || battler.effects[PBEffects::SyrupBomb] <= 0
      pbDisplay(_INTL("{1} got drenched in sticky syrup!", battler.pbThis))
      if battler.pbCanLowerStatStage?(:SPEED, battler, nil)
  ...
  def pbEOREndBattlerSelfEffects(battler)
    ...
    battler.effects[PBEffects::HamstringLock] -= 1 if battler.effects[PBEffects::HamstringLock] > 0
  end
  ...
  def pbEndOfRoundPhase
    ...
    pbTryCudChewEndOfRound(priority)
    pbGainExp
    return if @decision > 0
    # Form checks
  ...
  #-----------------------------------------------------------------------------
  # Cud Chew — berries replay at end of the round after the turn they were eaten
  #-----------------------------------------------------------------------------
  def suppress_cud_chew_register
    @cud_chew_register_depth = (@cud_chew_register_depth || 0) + 1
    yield
  ensure
    @cud_chew_register_depth -= 1
  end

  def pbCudChewRegisterSuppressed?
    (@cud_chew_register_depth || 0) > 0
  end

  def pbTryCudChewEndOfRound(priority)
    priority.each do |battler|
      next if battler.fainted?
      battler.pbExecuteCudChewReplay
    end
  end
end
