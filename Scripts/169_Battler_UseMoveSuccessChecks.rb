class Battle::Battler
  ...
  def pbCanChooseMove?(move, commandPhase, showMessages = true, specialUsage = false)
    # Disable
    disabled_moves = @effects[PBEffects::DisableMove]
    is_disabled = false
    if disabled_moves.is_a?(Array)
      is_disabled = disabled_moves.include?(move.id)
    else
      is_disabled = (disabled_moves == move.id)
    end
    if is_disabled && !specialUsage
      if showMessages
  ...
  def pbSuccessCheckAgainstTarget(move, user, target, targets)
    ...
    if !(user.hasActiveAbility?(:UNSEENFIST) && move.contactMove?) && !move.pbBypassProtect?
      # Wide Guard
