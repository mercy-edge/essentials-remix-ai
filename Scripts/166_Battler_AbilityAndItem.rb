class Battle::Battler
  ...
  def pbHeldItemTriggered(item_to_use, own_item = true, fling = false)
    ...
    # Bug Bite/Pluck users trigger Symbiosis; skip during Cud Chew replay (forced berry, no real consume)
    pbSymbiosis if !own_item && !fling && !@battle.pbCudChewRegisterSuppressed?
    if !@battle.pbCudChewRegisterSuppressed?
      berry_id = item_to_use.is_a?(Symbol) ? item_to_use : item_to_use&.id
      if berry_id && GameData::Item.try_get(berry_id)&.is_berry? &&
         hasActiveAbility?(:CUDCHEW) && abilityActive?
        @effects[PBEffects::CudChewBerry]      = berry_id
        @effects[PBEffects::CudChewTriggerTurn] = @battle.turnCount + 1
        @effects[PBEffects::CudChewWasFling]  = fling
      end
    end
  end

  def pbExecuteCudChewReplay
    return if fainted?
    berry_id = @effects[PBEffects::CudChewBerry]
    trigger_turn = @effects[PBEffects::CudChewTriggerTurn]
    return if !berry_id || trigger_turn != @battle.turnCount
    return if !hasActiveAbility?(:CUDCHEW) || !abilityActive?
    item_data = GameData::Item.try_get(berry_id)
    return if !item_data || !item_data.is_berry?
    was_fling = @effects[PBEffects::CudChewWasFling]
    @effects[PBEffects::CudChewBerry]      = nil
    @effects[PBEffects::CudChewTriggerTurn] = -1
    @effects[PBEffects::CudChewWasFling]   = false
    @battle.pbShowAbilitySplash(self)
    @battle.pbDisplay(_INTL("{1}'s {2} made it use its {3} again!",
                           pbThis, abilityName, item_data.name))
    @battle.pbHideAbilitySplash(self)
    @battle.suppress_cud_chew_register do
      pbHeldItemTriggerCheck(berry_id, was_fling)
    end
    pbFaint if fainted?
  end
