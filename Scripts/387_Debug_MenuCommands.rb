...
# MapYamlBridge: registered here (not only from plugin) so it appears even if Data/PluginScripts.rxdata is stale.
MenuHandlers.add(:debug_menu, :map_yaml_bridge_export_all_json, {
  "name"        => _INTL("Export all map events to JSON"),
  "parent"      => :files_menu,
  "description" => _INTL("MapYamlBridge: writes PBS/MapEvents/map###.json from each Data/Map###.rxdata."),
  "effect"      => proc {
    if defined?(pbExportAllMapEventsJson)
      pbExportAllMapEventsJson
    else
      pbMessage(_INTL("MapYamlBridge plugin is not loaded. Check Plugins/MapYamlBridge and recompile plugins."))
    end
  }
})
