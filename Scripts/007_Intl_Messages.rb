...
module Translator
  ...
  def gather_script_and_event_texts
    ...
    begin
      ...
      # script texts from .rb files in Data/Scripts (older layout) or Scripts/ (root)
      if $RGSS_SCRIPTS.length == 1
        script_dir = FileTest.directory?("Data/Scripts") ? "Data/Scripts" : "Scripts"
        if FileTest.directory?(script_dir)
          Dir.all(script_dir).each do |script_file|
            if System.uptime - t >= 5
              t += 5
              Graphics.update
            end
