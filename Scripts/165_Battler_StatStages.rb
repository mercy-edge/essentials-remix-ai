class Battle::Battler
  ...
  def pbRaiseStatStage(stat, increment, user, showAnim = true, ignoreContrary = false)
    ...
    if increment > 0
      @battle.pbTryOpportunistAfterStatGain(self, stat, increment)
    end
    return true
  end
  ...
  def pbLowerStatStageBasic(stat, increment, ignoreContrary = false)
    ...
    if increment > 0
      ...
      @statsDropped = true
      @effects[PBEffects::HadStatLoweredThisBattle] = true
    end
    return increment
  end
  ...
  def pbLowerAttackStatStageIntimidate(user)
    ...
    if hasActiveAbility?(:GUARDDOG) && abilityActive?
      @battle.pbShowAbilitySplash(self)
      pbRaiseStatStageByAbility(:ATTACK, 1, self, false)
      @battle.pbHideAbilitySplash(self)
      return true
    end
