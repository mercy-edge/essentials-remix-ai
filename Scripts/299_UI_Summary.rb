...
class PokemonSummary_Scene
  ...
  STAT_VIEW_MODES   = [:stats, :ivs, :evs]

  def pbRankedMoveDescriptionForSummary(move)
    return move.description if !move
    return move.description if !defined?(SpellRanks) || !defined?(pbGetSpellRank)
    return move.description if !$PokemonGlobal
    move_id = move.id
    return move.description if !move_id || !SpellRanks::MOVE_RANK_POWERS[move_id]
    rank = pbGetSpellRank(move_id)
    return _INTL("Rank: {1}\n{2}", rank, move.description)
  end
  ...
  def pbToggleStatViewMode
    idx = STAT_VIEW_MODES.index(@statViewMode) || 0
    @statViewMode = STAT_VIEW_MODES[(idx + 1) % STAT_VIEW_MODES.length]
  end

  def pbStatViewModeLabel
    case @statViewMode
    when :ivs then return _INTL("View: IVs")
    when :evs then return _INTL("View: EVs")
    else           return _INTL("View: Stats")
    end
  end

  def pbSummaryStatValueFor(stat)
    case @statViewMode
    when :ivs
      return @pokemon.iv[stat].to_s
    when :evs
      return @pokemon.ev[stat].to_s
    else
      return case stat
             when :HP then sprintf("%d/%d", @pokemon.hp, @pokemon.totalhp)
             when :ATTACK then @pokemon.attack.to_s
             when :DEFENSE then @pokemon.defense.to_s
             when :SPECIAL_ATTACK then @pokemon.spatk.to_s
             when :SPECIAL_DEFENSE then @pokemon.spdef.to_s
             when :SPEED then @pokemon.speed.to_s
             else "0"
             end
    end
  end
  ...
  def drawPageThree
    ...
    textpos = [
      ...
      [pbSummaryStatValueFor(:HP), 462, 82, :right, Color.new(64, 64, 64), Color.new(176, 176, 176)],
      [_INTL("Attack"), 248, 126, :left, base, statshadows[:ATTACK]],
      [pbSummaryStatValueFor(:ATTACK), 456, 126, :right, Color.new(64, 64, 64), Color.new(176, 176, 176)],
      [_INTL("Defense"), 248, 158, :left, base, statshadows[:DEFENSE]],
      [pbSummaryStatValueFor(:DEFENSE), 456, 158, :right, Color.new(64, 64, 64), Color.new(176, 176, 176)],
      [_INTL("Sp. Atk"), 248, 190, :left, base, statshadows[:SPECIAL_ATTACK]],
      [pbSummaryStatValueFor(:SPECIAL_ATTACK), 456, 190, :right, Color.new(64, 64, 64), Color.new(176, 176, 176)],
      [_INTL("Sp. Def"), 248, 222, :left, base, statshadows[:SPECIAL_DEFENSE]],
      [pbSummaryStatValueFor(:SPECIAL_DEFENSE), 456, 222, :right, Color.new(64, 64, 64), Color.new(176, 176, 176)],
      [_INTL("Speed"), 248, 254, :left, base, statshadows[:SPEED]],
      [pbSummaryStatValueFor(:SPEED), 456, 254, :right, Color.new(64, 64, 64), Color.new(176, 176, 176)],
      [pbStatViewModeLabel, 456, 28, :right, Color.new(64, 64, 64), Color.new(176, 176, 176)],
    ...
    pbDrawTextPositions(overlay, textpos)
    # Draw HP bar
    if @statViewMode == :stats && @pokemon.hp > 0
  ...
  def drawSelectedMove(move_to_learn, selected_move)
    ...
    pbDrawImagePositions(overlay, imagepos)
    # Draw selected move's description (including spell rank if configured)
    drawTextEx(overlay, 4, 224, 230, 5, pbRankedMoveDescriptionForSummary(selected_move), base, shadow)
  end
  ...
  def pbOptions
    ...
    if cmdGiveItem >= 0 && command == cmdGiveItem
      ...
      oldname = (@pokemon.nicknamed?) ? @pokemon.name : ""
      newname = pbEnterPokemonName(_INTL("{1}'s nickname?", speciesname),
                                   0, Pokemon::MAX_NAME_SIZE, oldname, @pokemon)
  ...
  def pbScene
    ...
    loop do
      ...
      if Input.trigger?(Input::ACTION)
        if @page == 3 && !@pokemon.egg?
          pbPlayDecisionSE
          pbToggleStatViewMode
          dorefresh = true
        else
          pbSEStop
          @pokemon.play_cry
