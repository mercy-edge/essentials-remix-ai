class Battle::Move
  ...
  def pbCalcAccuracyModifiers(user, target, modifiers)
    ...
    if user.hasActiveAbility?(:MINDSEYE) && user.abilityActive?
  ...
  def pbBaseDamage(baseDmg, user, target)
    return baseDmg if !defined?(pbSpellRankedBasePowerForUser)
    return pbSpellRankedBasePowerForUser(@id, baseDmg, user)
  end
  ...
  def pbCalcDamageMultipliers(user, target, numTargets, type, baseDmg, multipliers)
    ...
    Battle::Gen9AbilityEffects.apply_ruin_multipliers(self, user, target, multipliers, type)
    if damagingMove? && target.effects[PBEffects::GlaiveRushVulnerable]
    ...
    # Gen 9 — Electromorphosis / Wind Power (electric moves consume pending ×2 charge)
    if damagingMove? && type == :ELECTRIC
      if user.effects[PBEffects::ElectromorphosisCharge]
        multipliers[:power_multiplier] *= 2
        user.effects[PBEffects::ElectromorphosisCharge] = false
      elsif user.effects[PBEffects::WindPowerCharge]
        multipliers[:power_multiplier] *= 2
        user.effects[PBEffects::WindPowerCharge] = false
      end
    end
