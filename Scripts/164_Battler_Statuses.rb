class Battle::Battler
  ...
  def pbInflictStatus(newStatus, newStatusCount = 0, msg = nil, user = nil)
    ...
    # Poison Puppeteer — opposing Pokémon's poisoning moves also confuse the target
    if newStatus == :POISON && user && user.opposes?(self) &&
       user.hasActiveAbility?(:POISONPUPPETEER) && user.abilityActive? &&
       pbCanConfuse?(user, false)
      pbConfuse(_INTL("{1} became confused!", pbThis))
    end
    # Status cures
