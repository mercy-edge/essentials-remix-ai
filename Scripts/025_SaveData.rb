...
module SaveData
  ...
  # Returns nil if the file is missing, empty, or not valid Marshal (truncated/corrupt save).
  # @param file_path [String] path of the file to load from
  # @return [Hash, Array, nil] loaded save data
  ...
  def self.get_data_from_file(file_path)
    ...
    # Crashed mid-save, sync conflict, or truncated file ("marshal data too short")
    nil
  end
  ...
  def self.read_from_file(file_path)
    ...
    if save_data.nil?
      ...
      save_data = get_data_from_file(bak_path) if File.file?(bak_path)
    end
