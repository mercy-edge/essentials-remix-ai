class Battle::Move
  ...
  def pbBypassProtect?;   return false; end
  ...
  # Wind moves (for Wind Rider / Wind Power). Expand via PBS "Wind" flag or this list.
  def windMove?
    return true if @flags.any? { |f| f[/^Wind$/i] }
    return Battle::Move::GEN9_WIND_MOVE_IDS.include?(@id)
  end
  # Slicing moves (Sharpness). Expand via PBS "Slicing" flag or this list.
  def slicingMove?
    return true if @flags.any? { |f| f[/^Slicing$/i] }
    return Battle::Move::GEN9_SLICING_MOVE_IDS.include?(@id)
  end

  GEN9_WIND_MOVE_IDS = [
    :GUST, :WHIRLWIND, :HURRICANE, :HEATWAVE, :ICYWIND, :TAILWIND, :PETALBLIZZARD,
    :AIRSLASH, :AEROBLAST, :BLEAKWINDSTORM, :SANDSEARSTORM, :SPRINGTIDESTORM,
    :WINTERHOLDSTORM, :FAIRYWIND, :OMINOUSWIND, :RAZORWIND, :SILVERWIND
  ].freeze

  GEN9_SLICING_MOVE_IDS = [
    :AIRCUTTER, :AIRSLASH, :BEHEMOTHBLADE, :CEASELESSEDGE, :PSYBLADE,
    :CUT, :FURYCUTTER, :KOWTOWCLEAVE, :LEAFBLADE, :NIGHTSLASH, :PSYCHOCUT,
    :RAZORLEAF, :RAZORSHELL, :SLASH, :SOLARBLADE, :XSCISSOR, :HORNLEECH,
    :SMARTSTRIKE, :STONEAXE
  ].freeze
